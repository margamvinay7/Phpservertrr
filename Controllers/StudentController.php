            ...
            try {
                ...
                foreach ($data as $row) {
                    $student = [
                        ...
                        'address' => isset($row[8]) ? (string)$row[8] : null, // Assuming the seventh column contains addresses, with some cells possibly empty
                ...
                foreach ($studentData as $student) {
                    $query = "INSERT INTO Student (id, fullName, email, gender, mobile, joiningyear, address, year, academicyear) VALUES (:id, :fullName, :email, :gender, :mobile, :joiningyear, :address, :year, :academicyear)";
...
public function getStudentById() {
    try {
        ...
        // Prepare the SQL query
        $stmt = $pdo->prepare("SELECT id, fullName, rollNo, email, mobile, gender, year, joiningyear, address, academicyear, password, role FROM Student WHERE id = :id");
        $id = $_GET['id'];
        // Bind the parameter
